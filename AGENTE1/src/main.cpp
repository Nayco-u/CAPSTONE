...
const float KP = 5.0;      // Ganancia proporcional
const float KI = 10.0;       // Ganancia integral
...
void PWM_Control_Task(void *pvParameters) {
  ...
  while (true) {
    ...
    if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
      ...
      voltaje = v_barra / 5330.0; // Convertir a voltios
      xSemaphoreGive(dataMutex);
    }
    ...
    Serial.print(" V, Integral Error: ");
    Serial.print(integral_error, 2);
    Serial.print(" V, Duty: ");
    Serial.println(duty);
