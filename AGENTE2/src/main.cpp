...
void PWM_Control_Task(void *pvParameters) {
  ...
  while (true) {
    ...
    if (control_enabled){
      ...
      Serial.print("V, Voltaje Batería: ");
      Serial.print(ads2.computeVolts(v_cell1), 2);
      Serial.print(" V, SOC: ");
      Serial.print(soc / 100, 3);
      Serial.print("%, Corriente Batería: ");
      Serial.print(ads2.computeVolts(i_battery) / 0.103, 3);
      Serial.print("A, Corriente Carga: ");
      Serial.print(ads2.computeVolts(i_converter) / 0.103, 3);
      Serial.print(" A, Duty: ");
      Serial.println(duty);
    } else {
      // Control por SOC;
...
void Firebase_Update_Task(void *parameter) {
  ...
  while(true){
    ...
    if (app.ready()) {
      ...
      Database.set<object_t>(aClient, "/Agents/2", jsonData, processData, "🔐 Firebase_Update_Task");
